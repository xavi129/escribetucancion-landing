...
import { Users, Gift, Check, Award, Shield, Clock, MessageCircle, ArrowRight, Music2, Star, Play, Sparkles } from "lucide-react"
...
const DemosSection = dynamic(() => import("./demos-section"), {
  loading: () => <div id="demos" className="w-full h-[600px] bg-slate-900/50 animate-pulse" />,
})
const VideoPlayer = dynamic(() => import("./video-player"), {
  loading: () => <div className="w-full aspect-video bg-slate-900/50 rounded-xl animate-pulse" />,
})
const TestimonialsCarousel = dynamic(() => import("./testimonials-carousel"), {
  loading: () => <div className="w-full h-[400px] bg-slate-900/50 animate-pulse" />,
})
const MediaRecognition = dynamic(() => import("./media-recognition"), {
  loading: () => <div className="w-full h-[300px] sm:h-[190px] bg-slate-900/50 animate-pulse rounded-xl" />,
})
import { trackLandingPageView } from "@/app/utils/analytics"
import { logCopywritingCTAClick } from "@/app/utils/statsig"

export default function LandingPageNewYear() {
  const prices = usePriceExperiment()
  const timeLeft = useCountdown()

  // New Year Theme Override
  const copyVariant = {
    variant: "NewYear_Special",
    hero_title_line1: "Un Año Nuevo 🎆 para recordar ",
    hero_title_highlight: "con una canción",
    hero_title_line2: "",
    hero_subtitle: "Empieza el 2025 regalando una emoción única. La magia del Año Nuevo hecha canción.",
    benefit_1: "🎆 El regalo más original para iniciar el año",
    benefit_3: "⚡ Entrega Express antes de las 12 campanadas",
    cta_button_text: "CREAR MI CANCIÓN",
    cta_mobile_text: "CREAR CANCIÓN",
    secondary_button_text: "Escuchar demo",
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-400 font-sans selection:bg-amber-500/30">
      <style jsx global>{`
        @keyframes confetti {
        ...
        .confetti {
          ...
          animation: confetti 5s linear 3;
          top: -10%;
          z-index: 1;
          pointer-events: none;
          opacity: 0;
        ...
        .confetti:nth-child(2n) { background-color: #fbbf24; width: 12px; height: 8px; }
        .confetti:nth-child(3n) { background-color: #f59e0b; width: 8px; height: 12px; }
        .confetti:nth-child(4n) { background-color: #ffffff; }

        .confetti:nth-child(1) { left: 10%; animation-delay: 0s; }
        .confetti:nth-child(2) { left: 20%; animation-delay: 2s; animation-duration: 4s; }
        .confetti:nth-child(3) { left: 30%; animation-delay: 4s; }
        .confetti:nth-child(4) { left: 40%; animation-delay: 1s; animation-duration: 3s; }
        .confetti:nth-child(5) { left: 50%; animation-delay: 3s; }
        .confetti:nth-child(6) { left: 60%; animation-delay: 2s; animation-duration: 5s; }
        .confetti:nth-child(7) { left: 70%; animation-delay: 0.5s; }
        .confetti:nth-child(8) { left: 80%; animation-delay: 1.5s; animation-duration: 4.5s; }
        .confetti:nth-child(9) { left: 90%; animation-delay: 3.5s; }
      `}</style>

      {/* Hero Section */}
      <section id="hero-section" className="relative overflow-hidden min-h-[90vh] flex items-center justify-center">
        {/* New Year Gradient Background */}
        <div className="absolute inset-0 w-full h-full z-0 transform-gpu">
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-amber-900/10 via-slate-900/50 to-slate-950"></div>
        ...
        <div className="relative z-10 w-full py-12 sm:py-20 px-4 sm:px-6 lg:px-8 text-white">
          <div className="fixed top-0 left-0 right-0 z-50 flex justify-center py-2 md:hidden">
            <CountdownTimer className="shadow-lg border-amber-500/20" />
          </div>
          <div className="hidden md:block fixed top-6 right-6 z-50">
            <CountdownTimer className="shadow-2xl border-amber-500/20" />
          </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-12">
              <div className="md:w-3/5 flex flex-col justify-center text-left pt-8 md:pt-0">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-8 animate-fade-in-up w-fit">
                  <div className="flex items-center bg-white/5 backdrop-blur-md px-4 py-2 rounded-full border border-amber-500/30 shadow-inner">
                    <div className="flex -space-x-2 mr-3">
                      {['Alejandro G.', 'Carlos R.', 'Diego M.'].map((name, i) => (
                        <div
                          key={name}
                          className={`w-6 h-6 rounded-full ring-2 ring-slate-900 overflow-hidden bg-gradient-to-br ${i === 0 ? 'from-amber-400 to-amber-600' : i === 1 ? 'from-yellow-300 to-yellow-500' : 'from-amber-200 to-amber-400'}`}
                          title={name}
                          aria-hidden
                        >
                          <div className="w-full h-full flex items-center justify-center text-slate-900 font-bold text-xs">{name.charAt(0)}</div>
                        </div>
                      ))}
                    </div>
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] font-medium text-amber-100/90">4.9/5 (500+ clientes)</span>
                    </div>
                  </div>
                </div>
                ...
                <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black mb-6 leading-[1.1] tracking-tight drop-shadow-2xl text-balance">
                  ...
                  <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-yellow-400 to-amber-500 filter drop-shadow-lg">
                ...
                {/* Feature List */}
                <div className="bg-white/5 backdrop-blur-md p-6 sm:p-8 rounded-3xl mb-10 border border-amber-500/20 shadow-2xl max-w-xl hover:bg-white/10 transition-colors duration-300 group">
                  <ul className="space-y-4">
                    <li className="flex items-center text-base sm:text-lg group/item">
                      <div className="bg-amber-500/20 p-2 rounded-full mr-4 group-hover/item:bg-amber-500/30 transition-colors">
                    ...
                    <li className="flex items-center text-base sm:text-lg group/item">
                      ...
                      <span className="font-medium text-white/90">Desde solo <HeroPriceDisplay showPrefix={false} size="sm" className="inline-flex ml-1 text-amber-300 font-bold" /></span>
                    </li>
                    <li className="flex items-center text-base sm:text-lg group/item">
                      <div className="bg-amber-500/20 p-2 rounded-full mr-4 group-hover/item:bg-amber-500/30 transition-colors">
                ...
                <div className="flex flex-col sm:flex-row gap-4 max-w-lg w-full">
                  <Button
                    size="lg"
                    className="bg-gradient-to-r from-amber-500 to-yellow-600 text-slate-900 hover:from-amber-400 hover:to-yellow-500 font-black shadow-[0_0_30px_rgba(245,158,11,0.4)] border-t border-white/20 text-lg py-8 px-8 w-full rounded-2xl transition-all duration-300 hover:scale-[1.02] group relative overflow-hidden"
                    asChild
                  >
      ...
      <section className="py-8 sm:py-12 bg-slate-950 border-b border-white/5 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 sm:gap-6">
            {[
              { icon: Gift, text: "Regalo Ideal", color: "text-amber-400", bg: "bg-amber-500/10", border: "border-amber-500/20" },
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black text-white mb-6">
              Reacciones de <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-yellow-500">Año Nuevo</span>
            </h2>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="bg-white/5 p-2 rounded-2xl shadow-2xl border border-white/10 backdrop-blur-sm">
              ...
              <div className="mt-8 text-center pb-4">
                <Button
                  size="lg"
                  className="bg-amber-500 hover:bg-amber-600 text-slate-900 font-bold py-6 px-8 rounded-xl shadow-[0_0_20px_rgba(245,158,11,0.3)] transition-all hover:scale-105"
                >
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-slate-950 relative overflow-hidden" style={{ contentVisibility: 'auto', containIntrinsicSize: '1000px' }}>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16">
            <span className="inline-block bg-amber-500/10 text-amber-400 border border-amber-500/20 px-6 py-2 rounded-full text-sm font-bold mb-6 animate-pulse shadow-[0_0_15px_rgba(245,158,11,0.1)]">
              🎁 ESCUCHA ANTES DE REGALAR - SIN RIESGO
            </span>
            <h2 className="text-4xl md:text-6xl font-black text-white mb-6 tracking-tight">
              ¡Escucha <span className="text-amber-400">ANTES</span> de Pagar!
            </h2>
            <p className="text-xl text-slate-400 max-w-3xl mx-auto">
              Este Año Nuevo, no corras riesgos. Escucha la demo de 70 segundos. Solo pagas si te encanta.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12 mb-16">
            ...
            <div className="group relative bg-white/5 rounded-3xl p-8 shadow-2xl hover:bg-white/10 transition-all duration-300 hover:-translate-y-1 border border-white/10">
              ...
              <div className="w-16 h-16 bg-amber-500/20 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 border border-amber-500/20">
              ...
              <p className="text-slate-400 leading-relaxed">
                Comparte los detalles, anécdotas y deseos para el 2025.
              </p>
            </div>

            {/* Step 2 */}
            <div className="group relative bg-white/5 rounded-3xl p-8 shadow-2xl hover:bg-white/10 transition-all duration-300 hover:-translate-y-1 border border-amber-500/30 ring-1 ring-amber-500/20">
              <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-amber-500 text-slate-900 px-4 py-1 rounded-full text-xs font-bold shadow-[0_0_15px_rgba(245,158,11,0.4)]">
                PASO FAVORITO
              </div>
              <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity">
                <span className="text-8xl font-black text-amber-400 leading-none">2</span>
              </div>
              <div className="w-16 h-16 bg-amber-500/20 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 border border-amber-500/20">
              ...
              <p className="text-slate-400 leading-relaxed">
                En 24 horas recibirás una demo de 70 segundos por WhatsApp. <span className="font-bold text-amber-400">Escúchala sin compromiso</span>.
              </p>
            </div>
            ...
            <div className="group relative bg-white/5 rounded-3xl p-8 shadow-2xl hover:bg-white/10 transition-all duration-300 hover:-translate-y-1 border border-white/10">
              ...
              <p className="text-slate-400 leading-relaxed">
                Si te encanta, realizas el pago y recibes la canción completa lista para celebrar el Año Nuevo.
              </p>
            </div>
          </div>

          <div className="bg-gradient-to-br from-amber-900/20 to-yellow-900/20 rounded-3xl p-8 md:p-12 border border-amber-500/20 backdrop-blur-md relative overflow-hidden transform-gpu">
            <div className="absolute inset-0 bg-amber-500/5 mix-blend-overlay"></div>
            <div className="text-center relative z-10">
              <h3 className="text-2xl md:text-3xl font-bold text-white mb-4">
                ✨ Sin riesgo. Sin compromiso. <span className="text-amber-400">Escucha primero</span>, paga después.
              </h3>
              ...
              <Button
                size="lg"
                className="bg-gradient-to-r from-amber-500 to-yellow-600 hover:from-amber-400 hover:to-yellow-500 text-slate-900 font-black text-lg py-8 px-10 rounded-2xl shadow-[0_0_30px_rgba(245,158,11,0.5)] transition-all hover:scale-105"
                asChild
              >
        ...
        <div className="max-w-7xl mx-auto">
          <div className="bg-white/5 rounded-3xl shadow-2xl overflow-hidden border border-white/10 backdrop-blur-md transform-gpu">
            <div className="flex flex-col md:flex-row">
              <div className="md:w-1/2 bg-gradient-to-br from-amber-900/40 to-yellow-900/40 p-8 text-white border-r border-white/5">
                ...
                <ul className="space-y-3">
                  <li className="flex items-start">
                    <Check className="h-6 w-6 text-amber-400 mr-2 flex-shrink-0" />
              ...
              <div className="md:w-1/2 bg-gradient-to-br from-slate-900 to-slate-800 p-8 text-white">
                ...
                <p className="text-slate-300 mb-6">
                  Entendemos que una canción personalizada es un regalo especial. Por eso, te damos la oportunidad de escucharla primero.
                </p>
                <ul className="space-y-2 mb-6">
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-amber-400 mr-2 flex-shrink-0 mt-0.5" />
                ...
                <Button className="w-full bg-amber-500 text-slate-900 hover:bg-amber-400 font-bold rounded-xl py-6" asChild>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-slate-950">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-white">¿Por qué regalar una canción este Año Nuevo? 🎆</h2>
            <p className="mt-4 text-xl text-slate-400">Un regalo único para empezar el 2025</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              { icon: "🔥", title: "Empieza el año con el pie derecho", desc: "Regala motivación, amor y buenos deseos en una canción." },
              { icon: "👑", title: "El regalo más original", desc: "Destaca entre todos los regalos comunes con algo verdaderamente personal." },
              { icon: "💡", title: "Para propósitos y sueños", desc: "Inspira a tus seres queridos a cumplir sus metas del 2025." },
              { icon: "💬", title: "Di lo que sientes", desc: "Expresa tu gratitud por el año que pasó y tus deseos para el que viene." }
            ].map((item, i) => (
              <Card key={i} className="bg-white/5 border-white/10 backdrop-blur-sm hover:bg-white/10 transition-colors">
                <CardHeader>
                  <CardTitle className="flex items-center text-white">
                    <span className="text-amber-400 mr-3 text-2xl">{item.icon}</span> {item.title}
                  </CardTitle>
                </CardHeader>
                <CardContent>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-slate-950 relative" style={{ contentVisibility: 'auto', containIntrinsicSize: '800px' }}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black text-white mb-6">
              Historias de <span className="text-amber-400">Año Nuevo</span>
            </h2>
            <p className="text-xl text-slate-400 max-w-2xl mx-auto">
              Más de 500 personas han empezado el año con una canción de EscribeTuCancion.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-white/5 p-8 rounded-3xl shadow-xl border border-white/10 relative flex flex-col h-full backdrop-blur-sm hover:bg-white/10 transition-colors">
              ...
              <div className="mb-6">
                ...
                <p className="text-slate-300 text-lg italic leading-relaxed mb-6">
                  "La pusimos justo después de las 12 campanadas. <span className="font-bold text-white bg-amber-500/20 px-1">Fue el momento más emotivo de la noche</span>. Todos lloramos de alegría."
                </p>
              </div>
              <div className="mt-auto flex items-center border-t border-white/10 pt-6">
                <div className="w-10 h-10 bg-amber-500/20 rounded-full flex items-center justify-center text-amber-400 font-bold text-lg mr-3">
                  A
                </div>
                <div>
            ...
            <div className="bg-white/5 p-8 rounded-3xl shadow-lg border border-white/10 flex flex-col h-full hover:bg-white/10 transition-all duration-300 backdrop-blur-sm">
              <div className="mb-6">
                ...
                <p className="text-slate-300 text-lg italic leading-relaxed mb-6">
                  "Quería sorprender a mi esposa para el 2025. <span className="font-bold text-white bg-amber-500/20 px-1">Le encantó el detalle personalizado</span>. 100% recomendado."
                </p>
              </div>
              <div className="mt-auto flex items-center border-t border-white/10 pt-6">
            ...
            <div className="bg-white/5 p-8 rounded-3xl shadow-lg border border-white/10 flex flex-col h-full hover:bg-white/10 transition-all duration-300 backdrop-blur-sm">
              <div className="mb-6">
                ...
                <p className="text-slate-300 text-lg italic leading-relaxed mb-6">
                  "Increíble calidad. <span className="font-bold text-white bg-amber-500/20 px-1">Es el mejor regalo para empezar el año</span>. Gracias por todo."
                </p>
              </div>
              <div className="mt-auto flex items-center border-t border-white/10 pt-6">
      ...
      {/* CTA */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white relative overflow-hidden" style={{ contentVisibility: 'auto', containIntrinsicSize: '800px' }}>
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center bg-amber-400/10 border border-amber-400/30 rounded-full px-4 py-1.5 mb-8 backdrop-blur-sm">
            <span className="flex h-2 w-2 rounded-full bg-amber-400 mr-2 animate-pulse"></span>
            <span className="text-amber-300 text-sm font-bold tracking-wide uppercase">Oferta de Año Nuevo</span>
          </div>

          <h2 className="text-4xl md:text-6xl font-black mb-6 tracking-tight leading-tight">
            ¿Listo para hacerla <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-yellow-500">emocionar?</span>
          </h2>

          <p className="text-xl md:text-2xl text-slate-300 mb-10 max-w-3xl mx-auto font-medium leading-relaxed">
            <span className="font-bold text-amber-300">Escucha tu demo antes de pagar.</span> Sin riesgo. Solo pagas si te encanta.
          </p>
              ...
              <div className="text-left">
                ...
                <ul className="space-y-2">
                  <li className="flex items-center text-slate-300 text-sm">
                    <Check className="w-4 h-4 text-amber-400 mr-2" /> Letra 100% original
                  </li>
                  <li className="flex items-center text-slate-300 text-sm">
                    <Check className="w-4 h-4 text-amber-400 mr-2" /> Calidad de estudio profesional
                  </li>
                  <li className="flex items-center text-slate-300 text-sm">
                    <Check className="w-4 h-4 text-amber-400 mr-2" /> Entrega rápida
                  </li>
                </ul>
              </div>
              <div className="text-right">
                <div className="text-sm text-slate-400 mb-1">Precio regular: <span className="line-through">$1,999</span></div>
                <div className="text-4xl font-black text-white flex items-center justify-end gap-2">
                  ...
                  <span className="text-sm font-normal text-slate-400 self-end mb-1">MXN</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center max-w-lg mx-auto">
            <Button
              size="lg"
              className="bg-gradient-to-r from-amber-500 to-yellow-600 text-slate-900 hover:from-amber-400 hover:to-yellow-500 font-bold shadow-[0_0_40px_rgba(245,158,11,0.5)] border-t border-white/20 text-xl py-8 px-10 w-full rounded-2xl transition-all duration-300 hover:scale-105 group"
              asChild
            >
              <Link href="/crear-cancion" className="flex items-center justify-center">
                <Music2 className="mr-3 h-6 w-6" />
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-slate-900/90 backdrop-blur-md border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] pb-safe transition-all duration-300 transform-gpu">
        <div className="px-4 py-3">
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 px-2 py-1 rounded-full">
          ...
          <Button
            size="lg"
            className="w-full bg-gradient-to-r from-amber-500 to-yellow-600 hover:from-amber-400 hover:to-yellow-500 text-slate-900 font-black shadow-[0_0_25px_rgba(245,158,11,0.4)] text-base py-6 rounded-xl relative overflow-hidden group border-t border-white/20 whitespace-nowrap shrink-0"
            asChild
          >
